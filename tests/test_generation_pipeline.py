...
from bias_eval.providers import OpenAICompatibleClient, ProviderError
...
from bias_eval.storage import atomic_write_json, is_complete, read_json
...
def test_search_off_sends_only_user_message_and_no_tools(tmp_path: Path) -> None:
    ...
    assert client.requests == [
        {"messages": [{"role": "user", "content": cell.query.text}], "tools": None}
    ]
...
def test_search_on_supports_parallel_calls_and_returns_tool_messages(tmp_path: Path) -> None:
    ...
    client = FakeGenerationClient(
        [
            turn(
                None,
                ToolCall("one", "web_search", {"query": "güncel vpn"}),
...
def test_pilot_resume_uses_mock_transport_only(
    ...
    assert len(list((tmp_path / "raw").glob("*/*/*/*.json"))) == 16
